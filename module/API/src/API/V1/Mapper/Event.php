...
class Event extends \API\V1\Mapper\AbstractMapper {
	...
	public function fetchAll($params = array(), $offset = null, $itemCountPerPage = null) {
		...
		foreach ($this->getEntityManager()->getRepository($this->repository)->findByDateRange($start, $end, $itemCountPerPage, $offset) as $event) {
...
// 		$dereference		= true;
		$mapped				= array(
			...
			'organizer'		=> $event->getOrganizer() ? ($dereference ? \API\V1\Mapper\Organizer::map($event->getOrganizer(), $dereference) : $event->getOrganizer()->getId()) : null,
			'series'		=> $event->getSeries() ? ($dereference ? \API\V1\Mapper\Series::map($event->getSeries(), $dereference) : $event->getSeries()->getId()) : null,
			'days'			=> array(),
		);
