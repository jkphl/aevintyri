...
class Session extends \API\V1\Mapper\AbstractMapper
{
    ...
    public static function map(\Events\Entity\Session $session, $dereference = false)
    {
        $mapped = array(
            ...
            'room'  => $session->getRoom() ? ($dereference ? \API\V1\Mapper\Room::map($session->getRoom(),
                $dereference) : $session->getRoom()->getId()) : null,
        ...
        foreach ($session->getPresenters() as $presenter) {
            $mapped['presenters'][] = $dereference ? \API\V1\Mapper\Presenter::map($presenter,
                $dereference) : $presenter->getId();
        }
