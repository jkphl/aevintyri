...
class EntityRepository extends \Doctrine\ORM\EntityRepository {
	...
	public function getRelationCustomerContact($criteria) {
		...
		$q = $qb->select('p')->from('Relationship\Entity\Person', 'p')->innerJOIN('p.contact_customer_relations', 'r')->where('r.id = ' . $criteria['id'])->getQuery()->getResult();
		return $q;
	}
	...
	public function getRelationAgencyContact($criteria) {
		$em = $this->_em;
		$qb = $em->createQueryBuilder();
		$q = $qb->select('p')->from('Relationship\Entity\Person', 'p')->innerJOIN('p.contact_agency_relations', 'r')->where('r.id = ' . $criteria['id'])->getQuery()->getResult();
		return $q;
	}
}