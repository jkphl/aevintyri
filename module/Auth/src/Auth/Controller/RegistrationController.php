...
class RegistrationController extends AbstractActionController {
	...
	public function indexAction() {
		...
		if ($this->getRequest()->isPost()) {
			$form->setValidationGroup('username', 'email', 'name', 'password', 'passwordVerify', 'question', 'usergroup', 'answer', 'csrf', 'captcha');
			...
			if ($form->isValid()) {
				...
				try {
					...
					$this->sendEmail($user->getEmail(), $this->getTranslatorHelper()->translate('auth.register.confirm.email.subject'), sprintf($this->getTranslatorHelper()->translate('auth.register.confirm.email.body'), $fullLink));
					...
					return $this->getServiceLocator()->get('auth_error_view')->createErrorView($this->getTranslatorHelper()->translate('auth.register.confirm.email.failure'), $e, $this->getOptions()->getDisplayExceptions(), $this->getOptions()->getNavMenu());
				}
			}
		}
	...
	public function resetPasswordAction() {
		...
		if ($this->getRequest()->isPost()) {
			...
			if ($form->isValid()) {
				...
				$entityManager = $this->getEntityManager();
				$user = $entityManager->createQuery("SELECT u FROM Auth\Entity\User u WHERE u.email = '$usernameOrEmail' OR u.username = '$usernameOrEmail'")->getResult(\Doctrine\ORM\Query::HYDRATE_OBJECT);
				$user = $user[0];
				...
				if (isset($user)) {
					try {
						...
						$this->sendEmail($user->getEmail(), $this->getTranslatorHelper()->translate('auth.register.password.change.confirm.email.subject'), sprintf($this->getTranslatorHelper()->translate('auth.register.password.change.confirm.email.body'), $user->getUsername(), $fullLink));
						$entityManager->persist($user);
						$entityManager->flush();
						...
						return $this->getServiceLocator()->get('auth_error_view')->createErrorView($this->getTranslatorHelper()->translate('auth.register.password.change.confirm.email.failure'), $e, $this->getOptions()->getDisplayExceptions(), $this->getOptions()->getNavMenu());
					}
				} else {
		...
		if ($this->getRequest()->isPost()) {
			...
			if ($form->isValid()) {
				...
				if (UserCredentialsService::verifyHashedPassword($user, $this->params()->fromPost('password'))) {
					...
					$message = $this->getTranslatorHelper()->translate('auth.register.question.change.password.invalid');
				}
			}
		}
	...
	public function confirmEmailAction() {
		...
		try {
			...
			return $this->getServiceLocator()->get('auth_error_view')->createErrorView($this->getTranslatorHelper()->translate('auth.register.activate.failure'), $e, $this->getOptions()->getDisplayExceptions(), $this->getOptions()->getNavMenu());
		}
	}
	...
	public function confirmEmailChangePasswordAction() {
		...
		try {
			...
			if ($token !== '' && $user = $entityManager->getRepository('Auth\Entity\User')->findOneBy(array(
				...
				$this->sendEmail($user->getEmail(), $this->getTranslatorHelper()->translate('auth.register.email.change.email.subject'), sprintf($this->getTranslatorHelper()->translate('auth.register.email.change.email.body'), $user->getUsername(), $password, $fullLink));
			...
			return $this->getServiceLocator()->get('auth_error_view')->createErrorView($this->getTranslatorHelper()->translate('auth.register.email.change.failure'), $e, $this->getOptions()->getDisplayExceptions(), $this->getOptions()->getNavMenu());
		}
	}
	...
	private function sendEmail($to = '', $subject = '', $messageText = '') {
		...
		$message->addTo($to)->addFrom($this->getOptions()->getSenderEmailAdress())->setSubject($subject)->setBody($messageText);
