	...
	public function loginAction() {
		...
		if ($this->getRequest()->isPost()) {
			...
			if ($form->isValid()) {
				...
				try {
					$user				= $this->getEntityManager()->createQuery("SELECT u FROM Auth\Entity\User u WHERE u.email = '$usernameOrEmail' OR u.username = '$usernameOrEmail'")->getResult(\Doctrine\ORM\Query::HYDRATE_OBJECT);
					$user				= $user[0];
					...
					return $this->getServiceLocator()->get('auth_error_view')->createErrorView($this->getTranslatorHelper()->translate('auth.login.problem'), $e, $this->getOptions()->getDisplayExceptions(), $this->getOptions()->getNavMenu());
				}
			}
		}
