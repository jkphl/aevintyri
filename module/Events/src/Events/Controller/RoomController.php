...
class RoomController extends AbstractActionController {
	...
	public function addAction() {
		...
		$room								= new Room();
		$room->setVenue($this->getEntityManager()->getRepository('Events\Entity\Venue')->findOneBy(array('id' => $this->params('venue'))));
		$form->bind($room);
	...
	public function editAction() {
		...
		$room							= $this->getEntityManager()->getRepository('Events\Entity\Room')->findOneBy(array('id' => $this->params('id')));
		$form->bind($room);
		...
		if ($this->request->isPost()) {
			...
			if ($form->isValid()) {
				...
				if (array_key_exists('saveclose', $this->request->getPost())) {
					$this->redirect()->toRoute('venue', array('action' => 'edit', 'id' => $room->getVenue()->getId()), array('fragment' => 'rooms'));
				}
			}
		}
