...
class OrganizerController extends AbstractActionController {
	...
	public function addAction() {
		...
		if ($this->request->isPost()) {
			...
			if ($form->isValid()) {
				...
				} elseif (($imageInputFilter = $form->getInputFilter()->get('organizer')->get('upload_image')) && is_array($imageInputFilterValue = $imageInputFilter->getValue()) && array_key_exists('tmp_name', $imageInputFilterValue) && strlen($imageInputFilterValue['tmp_name'])) {
					$organizer->deleteImage();
				...
				if (intval($form->getInputFilter()->get('organizer')->get('delete_image')->getValue())) {
					$organizer->deleteImage();
					$form->get('organizer')->get('image')->setValue($organizer->getImage());
				} elseif (($imageInputFilter = $form->getInputFilter()->get('organizer')->get('upload_image')) && ($imageInputFilter->isValid())) {
					$organizer->setImage($imageInputFilter->getValue());
					$form->get('organizer')->get('image')->setValue($organizer->getImage());
				}
			}
	...
	public function editAction() {
		...
		$organizer							= $this->getEntityManager()->getRepository('Events\Entity\Organizer')->findOneBy(array('id' => $this->params('id')));
		$form->bind($organizer);
	...
	public function showAction() {
		...
		return array (
			'organizer'						=> $organizer,
			'identity'						=> $this->getIdentity(),
			'country'						=> $organizer->getCountry() ? $this->getEntityManager()->getRepository('Events\Entity\Country')->findOneBy(array('id' => $organizer->getCountry())) : null,
		);
	}
