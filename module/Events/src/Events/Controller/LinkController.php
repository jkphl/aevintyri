...
class LinkController extends AbstractActionController
{
    ...
    public function addAction()
    {
        ...
        if ($this->params('presenter')) {
            $link->setPresenter($this->getEntityManager()->getRepository('Events\Entity\Presenter')
                                     ->findOneBy(array('id' => $this->params('presenter'))));
        } elseif ($this->params('session')) {
            $link->setSession($this->getEntityManager()->getRepository('Events\Entity\Session')
                                   ->findOneBy(array('id' => $this->params('session'))));
        }
        $form->bind($link);
            ...
            if ($form->isValid()) {
                ...
                if (array_key_exists('createclose', $this->request->getPost())) {
                    if ($link->getPresenter()) {
                        $this->redirect()
                             ->toRoute('presenter', array('action' => 'edit', 'id' => $link->getPresenter()->getId()),
                                 array('fragment' => 'associations'));
                    } else {
                        $this->redirect()
                             ->toRoute('session', array('action' => 'edit', 'id' => $link->getSession()->getId()),
                                 array('fragment' => 'associations'));
                    }
                } else {
    ...
    public function editAction()
    {
        ...
        $link = $this->getEntityManager()->getRepository('Events\Entity\Link')
    ...
    public function deleteAction()
    {
        ...
        $this->persist($link);
        if ($link->getPresenter()) {
            $this->redirect()->toRoute('presenter', array('action' => 'edit', 'id' => $link->getPresenter()->getId()),
                array('fragment' => 'associations'));
        } else {
            $this->redirect()->toRoute('session', array('action' => 'edit', 'id' => $link->getSession()->getId()),
                array('fragment' => 'associations'));
        }
    }
}
