...
class VenueController extends AbstractActionController {
	...
	public function addAction() {
		...
		if ($this->request->isPost()) {
			...
			if ($form->isValid()) {
				...
				} elseif (($imageInputFilter = $form->getInputFilter()->get('venue')->get('upload_image')) && is_array($imageInputFilterValue = $imageInputFilter->getValue()) && array_key_exists('tmp_name', $imageInputFilterValue) && strlen($imageInputFilterValue['tmp_name'])) {
					$venue->deleteImage();
				...
				if (intval($form->getInputFilter()->get('venue')->get('delete_image')->getValue())) {
					$venue->deleteImage();
					$form->get('venue')->get('image')->setValue($venue->getImage());
				} elseif (($imageInputFilter = $form->getInputFilter()->get('venue')->get('upload_image')) && ($imageInputFilter->isValid())) {
					$venue->setImage($imageInputFilter->getValue());
					$form->get('venue')->get('image')->setValue($venue->getImage());
				}
			}
	...
	public function editAction() {
		...
		$venue							= $this->getEntityManager()->getRepository('Events\Entity\Venue')->findOneBy(array('id' => $this->params('id')));
		$form->bind($venue);
	...
	public function showAction() {
		...
		return array (
			'venue'						=> $venue,
			'identity'						=> $this->getIdentity(),
			'country'						=> $venue->getCountry() ? $this->getEntityManager()->getRepository('Events\Entity\Country')->findOneBy(array('id' => $venue->getCountry())) : null,
		);
	}
