...
class EventController extends AbstractActionController
{
    ...
    public function listAction()
    {
        ...
        return new ViewModel(array(
            'events' => $this->getEntityManager()
                             ->getRepository('Events\Entity\\'.(($type == 'series') ? 'Series' : 'Event'))->findAll(),
            'type'   => $type,
        ));
    }
    ...
    public function addAction()
    {
        ...
        if (($type == 'single') && $this->params('series')) {
            $series = $this->getEntityManager()->getRepository('Events\Entity\Series')
                           ->findOneBy(array('id' => $this->params('series')));
        ...
        if ($this->request->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                } elseif (($imageInputFilter = $form->getInputFilter()->get('event')
                ...
                if (array_key_exists('createclose', $this->request->getPost())) {
                    $this->redirect()
                         ->toRoute('event', array('action' => 'show', 'type' => $type, 'id' => $event->getId()));
                } else {
                    $this->redirect()
                         ->toRoute('event', array('action' => 'edit', 'type' => $type, 'id' => $event->getId()));
                }
            } else {
                if (intval($form->getInputFilter()->get('event')->get('delete_image')->getValue())) {
                    $event->deleteImage();
    ...
    public function editAction()
    {
        ...
        $event = $this->getEntityManager()
                      ->getRepository(($type == 'series') ? 'Events\Entity\Series' : 'Events\Entity\Event')
    ...
    public function deleteAction()
    {
        ...
        if ($type == 'series') {
            foreach (
                $this->getEntityManager()->getRepository('Events\Entity\Event')
                     ->findBy(array('series' => $event->getId())) as $subevent
            ) {
