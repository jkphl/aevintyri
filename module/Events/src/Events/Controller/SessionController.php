...
class SessionController extends AbstractActionController {
	...
	public function addAction() {
		$day								= $this->getEntityManager()->getRepository('Events\Entity\Day')->findOneBy(array('id' => $this->params('day')));
		...
		if ($this->request->isPost()) {
			...
			$form->setData(array_merge_recursive($this->request->getPost()->toArray(), $this->request->getFiles()->toArray()));
			...
			if ($form->isValid()) {
				...
				} elseif (($imageInputFilter = $form->getInputFilter()->get('session')->get('upload_image')) && is_array($imageInputFilterValue = $imageInputFilter->getValue()) && array_key_exists('tmp_name', $imageInputFilterValue) && strlen($imageInputFilterValue['tmp_name'])) {
					$session->deleteImage();
				...
				if (array_key_exists('createclose', $this->request->getPost())) {
					$this->redirect()->toRoute('event', array('action' => 'edit', 'type' => 'single', 'id' => $session->getDay()->getEvent()->getId()), array('fragment' => 'days'));
				} else {
				...
				if (intval($form->getInputFilter()->get('session')->get('delete_image')->getValue())) {
					$session->deleteImage();
					$form->get('session')->get('image')->setValue($session->getImage());
				} elseif (($imageInputFilter = $form->getInputFilter()->get('session')->get('upload_image')) && ($imageInputFilter->isValid())) {
					$session->setImage($imageInputFilter->getValue());
					$form->get('session')->get('image')->setValue($session->getImage());
				}
			}
	...
	public function editAction() {
		...
		$session							= $this->getEntityManager()->getRepository('Events\Entity\Session')->findOneBy(array('id' => $this->params('id')));
