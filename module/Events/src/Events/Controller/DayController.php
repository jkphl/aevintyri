...
class DayController extends AbstractActionController
{
    ...
    public function addAction()
    {
        ...
        $day->setEvent($this->getEntityManager()->getRepository('Events\Entity\Event')
                            ->findOneBy(array('id' => $this->params('event'))));
        $form->bind($day);
            ...
            if ($form->isValid()) {
                ...
                if (array_key_exists('createclose', $this->request->getPost())) {
                    $this->redirect()->toRoute('event',
                        array('action' => 'edit', 'type' => 'single', 'id' => $day->getEvent()->getId()),
                        array('fragment' => 'days'));
                } else {
    ...
    public function deleteAction()
    {
        ...
        $this->persist($day);
        $this->redirect()
             ->toRoute('event', array('action' => 'edit', 'type' => 'single', 'id' => $day->getEvent()->getId()),
                 array('fragment' => 'days'));
    }
}
