...
class PresenterController extends AbstractActionController {
	...
	public function addAction() {
		...
		if ($this->request->isPost()) {
			...
			if ($form->isValid()) {
				...
				} elseif (($imageInputFilter = $form->getInputFilter()->get('presenter')->get('upload_image')) && is_array($imageInputFilterValue = $imageInputFilter->getValue()) && array_key_exists('tmp_name', $imageInputFilterValue) && strlen($imageInputFilterValue['tmp_name'])) {
					$presenter->deleteImage();
				...
				if (intval($form->getInputFilter()->get('presenter')->get('delete_image')->getValue())) {
					$presenter->deleteImage();
					$form->get('presenter')->get('image')->setValue($presenter->getImage());
				} elseif (($imageInputFilter = $form->getInputFilter()->get('presenter')->get('upload_image')) && ($imageInputFilter->isValid())) {
					$presenter->setImage($imageInputFilter->getValue());
					$form->get('presenter')->get('image')->setValue($presenter->getImage());
				}
			}
	...
	public function editAction() {
		...
		$presenter							= $this->getEntityManager()->getRepository('Events\Entity\Presenter')->findOneBy(array('id' => $this->params('id')));
		$form->bind($presenter);
