	...
	protected function _persistImage($type) {
		if (strlen($this->image) && strncmp($this->image, '/img/'.$type, strlen($type) + 5)) {
			...
			if (@is_file($tmpFile)) {
				...
				if (@is_dir($appPath.'/public'.$imageDirectory) || (mkdir($appPath.'/public'.$imageDirectory) && chmod($appPath.'/public'.$imageDirectory, 0777))) {
					$image					= $imageDirectory.'/'.$type.'-'.$this->getId().'.'.pathinfo($this->image, PATHINFO_EXTENSION);
					if ((!@is_file($appPath.'/public'.$image) || @unlink($appPath.'/public'.$image)) && rename($tmpFile, $appPath.'/public'.$image)) {
						$this->image		= $image;
						return true;
					}
				}
