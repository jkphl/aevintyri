...
class Event extends AbstractEventEntity
{
    ...
    public function getDates()
    {
        ...
        return (($startDate instanceof \Events\Entity\Day) && ($endDate instanceof \Events\Entity\Day)) ? (($startDate->getDate()
                                                                                                                      ->format('Y-m-d') == $endDate->getDate()
                                                                                                                                                   ->format('Y-m-d')) ? array($startDate->getDate()) : array(
            $startDate->getDate(),
            $endDate->getDate()
        )) : array();
    }
    ...
    {
        return $this->days->filter(function ($entry) {
    ...
    public function getDateTimes()
    {
        ...
        if ($startDate instanceof \Events\Entity\Day) {
            ...
            if ($startSession instanceof \Events\Entity\Session) {
                $minutes = intval($startSession->getStart_time()
                                               ->format('G')) * 60 + intval(ltrim($startSession->getStart_time()
                                                                                               ->format('i'), '0'));
        ...
        if ($endDate instanceof \Events\Entity\Day) {
            ...
            if ($endSession instanceof \Events\Entity\Session) {
                $minutes = intval($endSession->getEnd_time()
                                             ->format('G')) * 60 + intval(ltrim($endSession->getEnd_time()->format('i'),
                        '0'));
                $endDateTime->add(new \DateInterval('PT'.$minutes.'M'));
            }
        }

        return (($startDateTime instanceof \DateTime) && ($endDateTime instanceof \DateTime)) ? (($startDateTime->format('r') == $endDateTime->format('r')) ? array($startDateTime) : array(
            $startDateTime,
            $endDateTime
        )) : array();
    }
